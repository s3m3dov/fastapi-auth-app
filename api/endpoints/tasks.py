...
@router.post('/task/')
def enqueue_user_ip_data(ip_address: str, Authorize: AuthJWT = Depends()):
    Authorize.jwt_required()

    db_ipdata = crud_ipdata.get_by_ip_address(ip_address)
    if db_ipdata:
        return {"msg": "The details of this IP exist in the database."}

    task = user_ip_data_save.delay(ip_address)
    return {"msg": "Task has been created.", "task_id": task.id}


@router.get("/status/{id}/")
async def check_task_status(task_id: str, Authorize: AuthJWT = Depends()):
    Authorize.jwt_required()
